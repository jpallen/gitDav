...
(function() {
    ...
    this.rm_r = function(path, callback) {
        this.git(['rm', '-r', path], callback);
    };
    
    this.mv = function(source, destination, callback) {
        this.git(['mv', source, destination], callback);
    };
    ...
    this.mvAndCommit = function(source, destination, message, author, callback) {
        var _self = this;
        this.mv(source, destination, function(error, stdout, stderr) {
            if (error) {
                callback(error, stdout, stderr);
                return;
            }
