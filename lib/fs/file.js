...
(function() {
    ...
    this.put = function(data, type, cbfsput) {
        var path = this.path, git = this.git, author = this.author;
        Fs.writeFile(path, data, type || "utf8", function(error) {
            if (error) {
                cbfsput(error);
                return;
            }
            
            git.add(path, function(error, stdout, stderr) {
                if (error) {
                    cbfsput(error);
                    return;
                }
                
                git.commit("Updated " + path, author, cbfsput);
            });
        });
    ...
    this["delete"] = function(cbfsfiledel) {
        var path = this.path, git = this.git, author = this.author;
        git.rm(path, function(error, stdout, stderr) {
            if (error) {
                console.log(error);
                cbfsfiledel(error);
                return;
            ...
            git.commit("Deleted " + path, author, cbfsfiledel);
        });
    };
